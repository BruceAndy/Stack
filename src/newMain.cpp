...
class Stack
{
    public:
        ...
        double Pop (int* errcode);
        double Peek(int* errcode);
        int getSize();
        int getCapacity();
        int Empty();
        int Print();
...
Stack::Stack(size_t capacity):
    ...
    canary2_ (PSN_INT)
    {
        data_ = (double*) malloc ((capacity + 2) * sizeof(double));
        data_canary_left_ = data_;
        data_canary_right_ = data_ + capacity_ + 1;
        *data_canary_left_ = *data_canary_right_ = 3.14159265;
        data_++;
...
{ 
	...
	*errcode = STKERR_OK; 
	double popped_data = data_[--counter_]; 
	check_sum_ = getCheckSum(); 
	IsOk(); 
	return popped_data; 
}

double Stack::Peek(int* errcode) { IsOk(); if ( !getSize() ) { printf("There's nothing to peek\n"); *errcode = STKERR_EMPTY; return -1; } *errcode = STKERR_OK; double popped_data = data_[counter_ - 1]; check_sum_ = getCheckSum(); IsOk(); return popped_data; }
# 140 "main.cpp"
int Stack::Add(int* errcode) { IsOk(); if (counter_ <= 1) { printf("Cannot perform addition.\n"); *errcode = STKERR_EMPTY; return -1; } *errcode = STKERR_OK; Push(Pop(errcode) + Pop(errcode)); check_sum_ = getCheckSum(); return IsOk(); }
int Stack::Mul(int* errcode) { IsOk(); if (counter_ <= 1) { printf("Cannot perform multiplication.\n"); *errcode = STKERR_EMPTY; return -1; } *errcode = STKERR_OK; Push(Pop(errcode) * Pop(errcode)); check_sum_ = getCheckSum(); return IsOk(); }
int Stack::Sub(int* errcode) { IsOk(); if (counter_ <= 1) { printf("Cannot perform subtraction.\n"); *errcode = STKERR_EMPTY; return -1; } *errcode = STKERR_OK; Push(Pop(errcode) - Pop(errcode)); check_sum_ = getCheckSum(); return IsOk(); }
int Stack::Div(int* errcode) { IsOk(); if (counter_ <= 1) { printf("Cannot perform division.\n"); *errcode = STKERR_EMPTY; return -1; } *errcode = STKERR_OK; Push(Pop(errcode) / Pop(errcode)); check_sum_ = getCheckSum(); return IsOk(); }
...
int Stack::Dump_(const char stackName[100])
{
    printf("================================================\n");
    IsOk ();
    ...
    printf("\tcounter_ is           %i\n", counter_ );
    printf("\tcapacity_ is          %i\n", capacity_ );
    printf("\tdata_ is              %p\n", data_ );
    printf("\tcheck_sum_ is         %lg\n", check_sum_ );
    printf("\tcanary2_ is           %i\n\n", canary2_ );
    printf("\tStack content:\n");
    for (size_t i = 0; i < capacity_; i++)
        printf("\t\t%c [%i]: %lg\n", (i < counter_)?42:32, i, data_[i]);
    printf("\t}\n");
    printf("================================================\n");
    IsOk();
    return 1;
...
int Stack::Extend()
{
    IsOk();
    capacity_ *= 2;
    data_ = (double*) realloc ( --data_ , (capacity_ + 2) * sizeof (*data_) );
    if (data_ == 
# 229 "main.cpp" 3 4
                __null
# 229 "main.cpp"
...
int Stack::IsOk()
{
    ...
    if (*data_canary_left_ != 3.14159265)
    {
        printf("data_canary_left_ in stack calls.\n");
        exit(1);
    }
    if (*data_canary_right_ != 3.14159265)
    {
...
void Stack::CountExpression(int length, char* express, int* errcode)
{
    ...
    while (i < length)
    {
        ...
        switch(express[i])
        {
            ...
            case '+': Add(errcode); break;
            case '-': Sub(errcode); break;
            case '*': Mul(errcode); break;
            case '/': Div(errcode); break;
...
{
    int n ;
    cout<< "Enter beginning size "<< endl;
    cin>> n;
